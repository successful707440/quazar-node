...
#[tokio::test]
async fn api_keys_role_matches_citizens_after_sync() {
    use crate::auth::{KeyStore, Role, MASTER_NAME};
    ...
    let citizen_id = format!("citizen-role-sync-{}", uuid::Uuid::new_v4());
    let api_key = format!("role_sync_key_{}", uuid::Uuid::new_v4().simple());

    sqlx::query(
        "INSERT INTO citizens (id, name, public_key, status, role, created_at, passport_issued)
         VALUES ($1, $2, $3, 'active', 'Aiya', EXTRACT(EPOCH FROM NOW())::BIGINT, TRUE)
         ON CONFLICT (name) DO UPDATE SET role = 'Aiya', status = 'active', passport_issued = TRUE",
    )
    .bind(&citizen_id)
    .bind(&citizen_name)
    .bind("role-sync-test-pubkey")
    .execute(&pool)
    .await
    .expect("insert citizen");

    KeyStore::upsert_key(&pool, &api_key, Role::Citizen, &citizen_name)
        .await
        .expect("insert mismatched api key");

    let key_role_before: String =
        sqlx::query_scalar("SELECT role FROM api_keys WHERE key = $1")
            .bind(&api_key)
            .fetch_one(&pool)
            .await
            .expect("key role before");
    assert_eq!(key_role_before, "Citizen");

    let updated = KeyStore::sync_roles_from_citizens(&pool)
        .await
        .expect("sync roles");
    assert_eq!(updated, 1);

    let key_role_after: String =
        sqlx::query_scalar("SELECT role FROM api_keys WHERE key = $1")
            .bind(&api_key)
            .fetch_one(&pool)
            .await
            .expect("key role after");
    assert_eq!(key_role_after, "Aiya");

    let citizen_role: String =
        sqlx::query_scalar("SELECT role FROM citizens WHERE name = $1")
            .bind(&citizen_name)
            .fetch_one(&pool)
            .await
            .expect("citizen role");
    assert_eq!(key_role_after, citizen_role);

    if let Some(successful_role) = KeyStore::lookup_citizen_role(&pool, MASTER_NAME).await {
        KeyStore::upsert_key(&pool, "successful_role_probe_key", Role::Citizen, MASTER_NAME)
            .await
            .expect("insert successful probe key");
        KeyStore::sync_roles_from_citizens(&pool)
            .await
            .expect("sync successful");
        let synced: String = sqlx::query_scalar(
            "SELECT role FROM api_keys WHERE key = 'successful_role_probe_key'",
        )
        .fetch_one(&pool)
        .await
        .expect("successful key role");
        assert_eq!(synced, successful_role.as_str());
    }

    let _ = sqlx::query("DELETE FROM api_keys WHERE key = $1")
        .bind(&api_key)
        .execute(&pool)
        .await;
    let _ = sqlx::query("DELETE FROM api_keys WHERE key = 'successful_role_probe_key'")
        .execute(&pool)
        .await;
    let _ = sqlx::query("DELETE FROM citizens WHERE id = $1")
        .bind(&citizen_id)
        .execute(&pool)
        .await;
}
