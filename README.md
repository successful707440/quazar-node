...
| `GMAIL_USER` | Gmail-адрес отправителя (например `quazarvs@gmail.com`) |
| `GMAIL_APP_PASSWORD` | [App Password](https://myaccount.google.com/apppasswords) для SMTP |
| `GMAIL_FROM_NAME` | Имя отправителя в письме (default: `Квазар`) |
...
- `POST /keys/sync` — выровнять `api_keys.role` по реестру `citizens` (Aiya/master); возвращает `{ "updated": N }`
- `POST /auth/login`, `GET /auth/check` — публичный вход по паролю и проверка наличия пароля
- `POST /auth/register/send-code`, `POST /auth/register/verify-code` — отправка и проверка кода регистрации с лендинга (Gmail SMTP, см. `GMAIL_*`)
- `POST /auth/set-password` — установка/смена пароля (требуется API-ключ гражданина)
...
- войти по паролю (`POST /auth/login`);
...
Ошибки для `pending`:
- **login**: «Вход недоступен: паспорт ещё не выдан (статус pending). Обратитесь к регистратору.»
- **API-ключ**: «Доступ запрещён: паспорт ещё не выдан (статус pending). Дождитесь подтверждения в блокчейне.» (**403**)

### Вход по паролю

Гражданин вводит имя и пароль; сервер возвращает API-ключ для дальнейших запросов.

```bash
curl -s "http://localhost:8080/auth/check?name=alice" | jq .

curl -s -X POST http://localhost:8080/auth/set-password \
...
curl -s -X POST http://localhost:8080/auth/login \
  -H "Content-Type: application/json" \
  -d '{"name": "alice", "password": "123456"}' | jq .
```
